...
RKNN_ROOT = $(CURDIR)/third_party/rknn_runtime
RKNN_INC = $(RKNN_ROOT)/include
RKNN_LIB = $(RKNN_ROOT)/lib
# 2. 【链接时】库文件目录：指向你从板子拉回来的克隆目?
SYSROOT_LIB = /home/c/rk3588_sysroot/usr/lib/

# 加入头文件搜索路? (如果你之前编? MPP 的头文件在别处，请在这里? -I 加上)
CFLAGS = -Wall -O2 -g -I$(FFMPEG_INC) -I$(RGA_INC) -I$(ELF2_EXT_INC) -I$(ELF2_EXT_ARCH_INC) -I$(RKNN_INC) -Wl,--allow-shlib-undefined
...
SRCS = main.c camera.c ipc.c encoder.c aliyun_mqtt.c sensor_modbus.c local_store.c video_store.c video_uploader.c relay_alarm.c rknn_worker.c
OBJS = $(SRCS:.c=.o)
