...
- 父进程同时维护推流子进程状态，负责在推流掉线后定时重启子进程
...
`main.c` 里的父子进程推流链路当前采用“掉线降级、定时重启”的策略：
...
- 当前 RTMP 正式链路已经改成：推流断开时子进程可以退出，但父进程不会因为 `send_fd/ack` 失败直接结束，而是继续采集并定时重启推流子进程。
- 如果主程序日志里已经出现：
